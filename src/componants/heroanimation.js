 ...
 *
 * Hello 👋
 * The SVG is generated by  polygonize.net/
 * http://polygonize.net/
 * The SVG Paths are morphed  with the CSS property transform.
 * I used transitions between classes to do the animations. The only animated css attributes is transform .
 * This is hardware accelerated. It only needs to paint two layers, the rest is done by the GPU.
 * That's the reason in still performs.
 * Due to using CSS transitions, I was forced to use CSS classes. I generated the classes randomly.
 * Each SVG path had three classes .cl-x  * scatters the paths randomly,
 * .cl-x.inital transforms back to 0 and .cl-x.reverse scatters it again randomly.
 * I just toggle classes with react afterwards..
 * I probably could saved one of the classes but as it is CSS I was happy enough it turned out alright,
 * so I didn't bother optimizing it more ;)
 *
 * You can find me on
 * Twitter: https://twitter.com/pascal_senn
 *
 *
 */
...
function Heroanimation() {
  ...
  return (
    <div className="" onClick={() => setReverse(!reverse)}>
      <svg
